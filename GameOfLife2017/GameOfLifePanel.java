...
{  
	...
	{
		int counter =  0; 
		if (r>0 && gameGrid[r-1][c]) counter++;
		if (r<gameGrid.length-1 && gameGrid[r+1][c]) counter++;
		if (r<gameGrid.length-1 && c<0 && gameGrid[r+1][c-1]) counter++;
		if (c<gameGrid[0].length-1 && gameGrid[r][c+1]) counter++;
		if (r>0 && c>0 && gameGrid[r-1][c-1]) counter++;
		if (r<gameGrid.length-1 && c>0 && gameGrid[r+1][c-1]) counter++;
		if (r>0 && c<gameGrid[0].length-1 && gameGrid[r-1][c+1]) counter++;
		if (r<gameGrid.length-1 && c<gameGrid[0].length-1 && gameGrid[r+1][c+1]) counter++;
